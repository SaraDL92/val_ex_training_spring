...
public interface RatingRepository extends JpaRepository<RatingEntity, Long>{
	...
	@Query("SELECT r FROM RatingEntity r WHERE idUserRating= :value1 AND ratingType LIKE '% :value2 %'")
	List<RatingEntity> findByNameRating (@Param("value1") Long idUser, @Param("value2") String nameRating);

	@Query("UPDATE RatingEntity r SET ratingType=:value1, rating= :value2 WHERE idRating= :value3")
